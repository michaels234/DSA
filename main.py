...
class Algorithm():
	def __init__(self, function, checker):
...
class BigO():
	...
	def start(self, n):
		# start BigO, set size n, save orders array
		self.n = n
		self.orders = [
	...
	def add(self):  # add to steps count
		self.count += 1
...
def sort_checker(o, function, min_size=100, max_size=1000, seed=None, prnt=None):
	arr = make_array(min_size, max_size, seed)
	o.start(n=len(arr))
	start_arr = arr.copy()
	result = function(o, arr)
...
#def run(o, arr):
#	arr = arr.copy()  # to avoid altering the input arr externally
#	return algorithm(o, arr)


def test_all(algo_list):
	seed = random.randrange(0, 100)
	for algorithm in algo_list:
		o = BigO()
		print_data = PrintData()
		prnt = algorithm.function.__name__
		algorithm.checker(o, algorithm.function, min_size=10, max_size=10, seed=seed, prnt=prnt)
		order_level = round(o.get_order_level(prnt=prnt), 3)
		order_int = math.ceil(order_level)
		order_level_str = str(order_level)
		print_data.add([
			algorithm.function.__name__,
			o.orders[order_int]['name'] + '\t',
			o.orders[order_int-1]['name'],
...
def run_all(run_list):
	# run all algorithms 20 times each, count steps and get BigO results. and save results to be printed out at the end
	run_times = 50
	...
	for algorithm in run_list:  # for every algorithm in run_list
		...
		for i in range(run_times):  # run times
			o = BigO()  # BigO is used to count steps over time and get the order level etc. at the end
			algorithm.checker(o, algorithm.function, seed=seed_list[i])
			order_level = o.get_order_level()
...
def simple_sort(o, arr):
	...
	for i in range(len(arr) - 1):  # for every element in arr (can skip last one, because all will be set at that point)
		min_j = None  # every time, reset the min_j, the index of the min element, which we will search for
		for j in range(i, len(arr)):  # check all from i (earlier i have already been set)
			o.add()
			if min_j is None or arr[j] < arr[min_j]:  # set min_j = j if arr[j] < arr[min_j] OR if min_j not set yet
				min_j = j
...
def merge_sort(o, arr):
	...
	if n <= 1:  # if down to 1-element array, it is sorted, just set it, this ends the recursion
		...
		arr_a = merge_sort(o, arr[:a])
		arr_b = merge_sort(o, arr[a:])
